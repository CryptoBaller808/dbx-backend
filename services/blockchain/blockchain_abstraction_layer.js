...
class ConfigurationManager {
  ...
  async loadConfigurations() {
    const configs = await this.blockchainConfigRepository.findAll({
      where: { isActive: true }
    });
    ...
    if (sourceChainId === targetChainId) {
      try {
        ...
        const signedTx = await this.transactionService.signTransaction(
          sourceChainId,
          tx,
          swapParams.signingParams
        );
        
        // Submit the transaction
        return await this.transactionService.submitTransaction(sourceChainId, signedTx);
      } catch (error) {
        if (error instanceof BlockchainError) {
          throw error;
        }
...
/**
 * Initialize the blockchain abstraction layer
 * @param {Object} blockchainConfigRepository - Repository for blockchain configurations
 * @param {Object} transactionRepository - Repository for transactions
 * @returns {Promise<Object>} Initialized services
 */
async function initializeBlockchainLayer(blockchainConfigRepository, transactionRepository) {
  // Load configurations
  const configManager = new ConfigurationManager(blockchainConfigRepository);
  ...
  for (const [chainId, config] of configManager.configs) {
    let adapter;
    
    // Create appropriate adapter based on configuration
    switch (config.adapterType) {
      case 'xrp':
        const XRPAdapter = require('./adapters/xrp.adapter');
        adapter = new XRPAdapter(config);
        break;
      case 'stellar':
        const StellarAdapter = require('./adapters/stellar.adapter');
        adapter = new StellarAdapter(config);
        break;
      case 'xdc':
        const XDCAdapter = require('./adapters/xdc.adapter');
        adapter = new XDCAdapter(config);
        break;
      case 'solana':
        const SolanaAdapter = require('./adapters/solana.adapter');
    ...
    try {
      ...
      adapterRegistry.registerAdapter(chainId, adapter);
      console.log(`Initialized adapter for chain: ${chainId}`);
    } catch (error) {
      console.error(`Failed to initialize adapter for chain ${chainId}:`, error);
      // Continue with other adapters even if one fails
    }
  }
  
  // Create service instances
  const blockchainService = new BlockchainService(adapterRegistry);
  const walletOperations = new WalletOperations(adapterRegistry);
  const transactionService = new TransactionService(adapterRegistry, transactionRepository);
  const swapOperations = new SwapOperations(adapterRegistry, transactionService);
