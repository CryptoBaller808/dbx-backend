...
class AVAXAdapter extends BlockchainAdapter {
  ...
  async connectToRpc(rpcUrl) {
    try {
      console.log(`[AVAX Adapter] Connecting to RPC: ${rpcUrl}`);
      ...
      console.log(`[AVAX Adapter] Connected to network: ${network.name} (Chain ID: ${network.chainId})`);
      ...
      throw new BlockchainError(
        `Failed to connect to AVAX RPC: ${error.message}`,
        ErrorCodes.CONNECTION_ERROR,
        this.chainId,
        error
      );
