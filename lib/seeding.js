 ...
 */
function validateRequiredColumns(requiredNoDefault, insertValues) {
  console.log('[SEED] Validating required columns coverage...');
  
  const providedColumns = new Set(Object.keys(insertValues));
  const missing = requiredNoDefault.filter(col => !providedColumns.has(col));
  ...
  console.log('[SEED] ✅ All required columns covered');
}
...
async function ensureRolesAndAdmin({ sequelize, email, plainPassword, roleName = 'Admin', transaction }) {
  ...
  try {
    ...
    validateRequiredColumns(requiredNoDefault, insertValues);
    
    // Step 7: Build SQL queries
    ...
    // Step 8: Execute UPDATE-then-INSERT pattern
