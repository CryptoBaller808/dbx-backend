...
function createMigrationRunner(sequelize) {
  ...
  // Fix Umzug v3 context wiring as specified by Owen
...
async function baselineAndRunAllowlist(sequelize, { allowlist = [] } = {}) {
  try {
    ...
    if (allowlistMigrations.length > 0) {
      ...
      // Add sanity log as requested by Owen
      console.info('[MIGRATION] ctx hasQI=%s hasSequelize=%s',
        !!umzug.context, !!umzug.context?.sequelize?.query);
      
      for (const migrationName of allowlistMigrations) {
        try {
          // Before execution breadcrumb as requested by Owen
          console.info('[MIGRATION] executing', migrationName, { 
            hasQI: !!umzug.context?.sequelize, 
            hasSequelize: !!umzug.context?.sequelize?.query 
          });
