...
// Health endpoint with CORS info and database schema diagnostics
  ...
  try {
    ...
    try {
      ...
      if (tableCheck.length > 0) {
        ...
        healthResponse.dbSchemaDiagnostics.tables.roles.admin_role_exists = rolesData.some(role => role.name === 'admin');
        healthResponse.dbSchemaDiagnostics.tables.roles.role_id_2_exists = rolesData.some(role => role.id === 2);
        
        // Check if admin role with id 2 exists
        const adminRoleWithId2 = rolesData.find(role => role.id === 2);
        if (adminRoleWithId2) {
          healthResponse.dbSchemaDiagnostics.tables.roles.admin_role_id_2 = adminRoleWithId2;
        }
      } else {
